...
public class UrchinColonyFeature extends Feature<SimpleBlockFeatureConfig> {
    public UrchinColonyFeature(Codec<SimpleBlockFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<SimpleBlockFeatureConfig> context) {
        ...
        if (blockState.canPlaceAt(world, blockPos) && world.getBlockState(blockPos) == Blocks.WATER.getDefaultState()) {
            ...
            if (world.getBlockState(testPos).isOf(Blocks.WATER)) {
                world.setBlockState(testPos, TidesBlocks.URCHIN_SPINES.getDefaultState().with(MarinalOrganBlock.WATERLOGGED, true),1);
            }
            for (int i = 0; i < 5; i++) {
                if (random.nextInt(2) == 0) {
                    if (i == 0) {
                        world.setBlockState(blockPos.north(), TidesBlocks.URCHIN_SPINES.getDefaultState().with(MarinalOrganBlock.WATERLOGGED, true)
                                .with(MarinalOrganBlock.FACING, Direction.NORTH), 1);
                    } else if (i == 1) {
                        world.setBlockState(blockPos.east(), TidesBlocks.URCHIN_SPINES.getDefaultState().with(MarinalOrganBlock.WATERLOGGED, true)
                                .with(MarinalOrganBlock.FACING, Direction.EAST), 1);
                    } else if (i == 2) {
                        world.setBlockState(blockPos.south(), TidesBlocks.URCHIN_SPINES.getDefaultState().with(MarinalOrganBlock.WATERLOGGED, true)
                                .with(MarinalOrganBlock.FACING, Direction.SOUTH), 1);
                    } else if (i == 3) {
                        world.setBlockState(blockPos.west(), TidesBlocks.URCHIN_SPINES.getDefaultState().with(MarinalOrganBlock.WATERLOGGED, true)
